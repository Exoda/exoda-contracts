...
library ExodaERC20 {
	...
	bytes4 private constant _SIG_TOTALSUPPLY = 0x18160ddd; // balanceOf(address)
	...
	function safeTransfer(IERC20 token, address to, uint256 amount) internal {
		(bool success, bytes memory data) = address(token).call(
			abi.encodeWithSelector(_SIG_TRANSFER, to, amount)
		);
		require(
	...
	function safeSymbol(IERC20 token) internal view returns (string memory) {
		(bool success, bytes memory data) = address(token).staticcall(
			abi.encodeWithSelector(_SIG_SYMBOL)
		);
	...
	function safeName(IERC20 token) internal view returns (string memory) {
		(bool success, bytes memory data) = address(token).staticcall(
			abi.encodeWithSelector(_SIG_NAME)
		);
		return success ? returnDataToString(data) : "???";
	}
	...
	function safeDecimals(IERC20 token) internal view returns (uint8) {
		(bool success, bytes memory data) = address(token).staticcall(
			abi.encodeWithSelector(_SIG_DECIMALS)
		);
	...
	function safeTotalSupply(
		...
		(bool success, bytes memory data) = address(token).staticcall(
			abi.encodeWithSelector(_SIG_TOTALSUPPLY)
		);
		require(
