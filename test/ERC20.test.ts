...
describe("ERC20", () =>
{
	...
	context("this", async () =>
	{
		beforeEach(async () =>
		{
			ERC20 = (await ERC20Factory.deploy("Name", "SYM")) as IERC20Mock;
		...
		it("ERC20.constructor: Should emit nothing", async () =>
		{
			// Arrange
			// Act
			const result = ERC20.deployTransaction;
			// Assert
			await EmitOnlyThis(result, ERC20);
		});
		...
		it("ERC20._burn: Should allow burn of tokens", async () =>
		{
			...
			// Act
			const result = await ERC20.mockBurn(Alice.address, "10");
			// Assert
			await expect(result).to.emit(ERC20, "Transfer(address,address,uint256)").withArgs(Alice.address, ADDRESS_ZERO, 10);
		...
		it("ERC20._burn: Should not allow burn of zero address", async () =>
		{
			// Arrange
			// Act
			const result = ERC20.mockBurn(ADDRESS_ZERO, "1");
			// Assert
		...
		it("ERC20._burn: Should not allow burn more tokens than owned", async () =>
		{
			...
			await ERC20.mockMint(Alice.address, "100");
			// Act
			const result = ERC20.mockBurn(Alice.address, "101");
			// Assert
		...
		it("ERC20._mint: Should allow mint of tokens", async () =>
		{
			// Arrange
			// Act
			const result = await ERC20.mockMint(Alice.address, "1");
			// Assert
			await expect(result).to.emit(ERC20, "Transfer(address,address,uint256)").withArgs(ADDRESS_ZERO, Alice.address, 1);
			await EmitOnlyThis(result, ERC20, "Transfer(address,address,uint256)");
		...
		it("ERC20._mint: Should not allow mint on zero address", async () =>
		{
			// Arrange
			// Act
			const result = ERC20.mockMint(ADDRESS_ZERO, "1");
			// Assert
		...
		it("ERC20.transferFrom: Should allow token transfer and not reduce infinite allowance", async () =>
		{
			...
			await ERC20.mockMint(Alice.address, "100");
			await ERC20.increaseAllowance(Bob.address, max);
			// Act
			const result = await ERC20.connect(Bob).transferFrom(Alice.address, Carol.address, 10);
			// Assert
			const totalSupply = await ERC20.totalSupply();
		...
		it("ERC20.transferFrom: Should not allow transfer more than balance", async () =>
		{
			// Arrange
			await ERC20.mockMint(Alice.address, "100");
			await ERC20.increaseAllowance(Bob.address, 200);
			// Act
		...
		it("ERC20.transferFrom: Should not allow transfer more than allowance", async () =>
		{
			// Arrange
			await ERC20.mockMint(Alice.address, 100);
			await ERC20.increaseAllowance(Bob.address, 90);
			// Act
	...
	context("IERC20AltApprove", async () =>
	{
		...
		it("ERC20.decreaseAllowance: Should allow token holder to change allowance", async () =>
		{
			...
			// Assert
			await EmitOnlyThis(result, ERC20, "Approval(address,address,uint256)");
			expect(await ERC20.allowance(Alice.address, Bob.address)).to.equal(50);
		});
		...
		it("ERC20.decreaseAllowance: Should allow token holder to change allowance multible times", async () =>
		{
			...
			// Assert
			expect(await ERC20.allowance(Alice.address, Bob.address)).to.equal(20);
		});
		...
		it("ERC20.decreaseAllowance: Should not allow token holder to change allowance below 0", async () =>
		{
			...
			expect(await ERC20.allowance(Alice.address, Bob.address)).to.equal(100);
		});
		...
		it("ERC20.increaseAllowance: Should allow token holder to change allowance above hold tokens", async () =>
		{
			...
			await ERC20.mockMint(Alice.address, "100");
			// Act
			const result = await ERC20.increaseAllowance(Bob.address, 200);
			// Assert
			await EmitOnlyThis(result, ERC20, "Approval(address,address,uint256)");
			expect(await ERC20.allowance(Alice.address, Bob.address)).to.equal(200);
		});
		...
		it("ERC20.increaseAllowance: Should allow token holder to change allowance multible times", async () =>
		{
			...
			// Assert
			expect(await ERC20.allowance(Alice.address, Bob.address)).to.equal(80);
		});
	});
